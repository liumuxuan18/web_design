...
$(function(){
  ...
  loadImages(sources, function (images) {
    for (var x in mapArray) { //[XXX] 每一行(當一個元素)
      for (var y in mapArray[x]) { //每一行裡面的每一個元素
        if (mapArray[x][y] == 1) {
...
if(targetBlock.x != -1 && targetBlock.y !=-1){
  switch(mapArray[targetBlock.x][targetBlock.y]){
    ...
    case 1://有障礙物(不可移動)
      ...
      break;
    case 2://終點(可移動)
      $("#talkBox").text("抵達終點");currentImgMain.x=targetImg.x;currentImgMain.y=targetImg.y;
      break;
    case 3://敵人(不可移動)
      $("#talkBox").text("哈摟");
      break;}
